...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
      { var shapes = {  "ground": new Square(),
                        "tree_trunk": new Capped_Cylinder(60, 60),
                        "foliage": new Grid_Sphere(60, 60),
                        "head": new Grid_Sphere(60, 60),
                        "thorax": new Cube(),
                        "abdomen": new Grid_Sphere(60, 60),
                        "wings": new Cube(),
                        "leg_segment": new Cube() };

        this.submit_shapes( context, shapes );
        
        this.define_data_members( { grass: context.shaders_in_use["Phong_Model"].material( Color( 0.5, 1, 0, 1 ), .2, 1,  1, 40 ),
    ...
    'display'( graphics_state )
      {         
        ...
        model_transform = mult( model_transform, scale(60, 60, 60));
        this.shapes.ground          .draw( graphics_state, model_transform,                      this.grass   );

        //  tree
        model_transform = identity();
        model_transform = mult( model_transform, rotation(-90, 1, 0, 0));
        model_transform = mult( model_transform, translation(0, 0, -0.417));
        for(var i = 0; i < 8; i++) {
          model_transform = mult( model_transform, translation(0, 0, 0.417));
          model_transform = mult(model_transform, rotation(5.625 * Math.sin(graphics_state.animation_time/1000), 0, 1, 0));
          model_transform = mult( model_transform, translation(0, 0, 0.5));
          this.shapes.tree_trunk          .draw( graphics_state, model_transform,                      this.wood  );
        }
        model_transform = mult( model_transform, scale(6, 6, 6));
        model_transform = mult( model_transform, translation(0, 0, 0.833));
        this.shapes.foliage         .draw( graphics_state, model_transform,                      this.leaves  );

        //  bee
        model_transform = identity();
        model_transform = mult(model_transform, rotation(-0.01 * graphics_state.animation_time, 0, 1, 0));
        model_transform = mult(model_transform, translation(0, 6 + 1 * Math.sin(graphics_state.animation_time/1000), 12));
        var appendages_transform = model_transform;
        model_transform = mult(model_transform, scale(2, 1, 1));
        this.shapes.thorax.draw(graphics_state, model_transform, this.thorax);

        model_transform = mult(model_transform, scale(1/2, 1, 1));
        model_transform = mult(model_transform, translation(-3, 0, 0));
        this.shapes.head.draw(graphics_state, model_transform, this.head);

        model_transform = mult(model_transform, translation(7, 0, 0));
        model_transform = mult(model_transform, scale(3, 2, 2));
        this.shapes.abdomen.draw(graphics_state, model_transform, this.abdomen);

        var wings_transform = appendages_transform;
        wings_transform = mult(wings_transform, translation(0, 1, 0));

        var left_wing = wings_transform;
        left_wing = mult(left_wing, translation(0, 0, 1));
        left_wing = mult(left_wing, rotation(45 * Math.sin(graphics_state.animation_time/1000), 1, 0, 0));
        left_wing = mult(left_wing, translation(0, 1/12, 2));
        left_wing = mult(left_wing, scale(1, 1/12, 2));
        this.shapes.wings.draw(graphics_state, left_wing, this.wings);

        var right_wing = wings_transform;
        right_wing = mult(right_wing, translation(0, 0, -1));
        right_wing = mult(right_wing, rotation(-45 * Math.sin(graphics_state.animation_time/1000), 1, 0, 0));
        right_wing = mult(right_wing, translation(0, 1/12, -2));
        right_wing = mult(right_wing, scale(1, 1/12, 2));
        this.shapes.wings.draw(graphics_state, right_wing, this.wings);

        function draw_leg(transform, leg_segment, shader) {
          //  upper segment
          transform = mult(transform, rotation(45, 1, 0, 0));
          transform = mult(transform, rotation(-30 * Math.sin(graphics_state.animation_time/1000), 1, 0, 0));
          transform = mult(transform, translation(0, -1, -1/12));
          transform = mult(transform, scale(1/12, 1, 1/12));
          leg_segment.draw(graphics_state, transform, shader);

          //  lower segment
          transform = mult(transform, scale (12, 1, 12));
          transform = mult(transform, rotation(-45, 1, 0, 0));
          transform = mult(transform, translation(0, -0.762, -0.650));
          transform = mult(transform, rotation(30 * Math.sin(graphics_state.animation_time/1000), 1, 0, 0));
          transform = mult(transform, translation(0, -1, -1/12));
          transform = mult(transform, scale(1/12, 1, 1/12));
          leg_segment.draw(graphics_state, transform, shader);
        ...
        appendages_transform = mult(appendages_transform, translation(-1.5, -1, -1));
        draw_leg(appendages_transform, this.shapes.leg_segment, this.thorax);

        for(var i = 0; i < 2; i++) {
          appendages_transform = mult(appendages_transform, translation(1.5, 0, 0));
          draw_leg(appendages_transform, this.shapes.leg_segment, this.thorax);
        }

        appendages_transform = mult(appendages_transform, rotation(180, 0, 1, 0));
        appendages_transform = mult(appendages_transform, translation(0, 0, -2));
        draw_leg(appendages_transform, this.shapes.leg_segment, this.thorax);
...
Declare_Any_Class( "Custom_Animation",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
    ...
    'init_keys'( controls )   // init_keys():  Define any extra keyboard shortcuts here
      { 
      	//	pitch down
      	controls.add( "w", this, function() { this.objects[0].ang_vel[0] = -90; });
      	controls.add( "w", this, function() { this.objects[0].ang_vel[0] = 0; }, {'type':'keyup'} );
      	//	pitch up
      	controls.add( "s", this, function() { this.objects[0].ang_vel[0] = 90; });
      	controls.add( "s", this, function() { this.objects[0].ang_vel[0] = 0; }, {'type':'keyup'} );
      	//	yaw right
      	controls.add( "d", this, function() { this.objects[0].ang_vel[1] = -90; });
      	controls.add( "d", this, function() { this.objects[0].ang_vel[1] = 0; }, {'type':'keyup'} );
      	//	yaw left
      	controls.add( "a", this, function() { this.objects[0].ang_vel[1] = 90; });
      	controls.add( "a", this, function() { this.objects[0].ang_vel[1] = 0; }, {'type':'keyup'} );
      	// roll clockwise
      	controls.add( "e", this, function() { this.objects[0].ang_vel[2] = -90; });
      	controls.add( "e", this, function() { this.objects[0].ang_vel[2] = 0; }, {'type':'keyup'} );
      	//	roll counterclockwise
      	controls.add( "q", this, function() { this.objects[0].ang_vel[2] = 90; });
      	controls.add( "q", this, function() { this.objects[0].ang_vel[2] = 0; }, {'type':'keyup'} );
      	//	thrust forwards
      	controls.add( "h", this, function() { this.objects[0].acceleration[2] = -10; });
      	controls.add( "h", this, function() { this.objects[0].acceleration[2] = 0; }, {'type':'keyup'} );
      	//	thrust backwards
      	controls.add( "n", this, function() { this.objects[0].acceleration[2] = 10; });
      	controls.add( "n", this, function() { this.objects[0].acceleration[2] = 0; }, {'type':'keyup'} );
      	//	thrust down
      	controls.add( "i", this, function() { this.objects[0].acceleration[1] = -10; });
      	controls.add( "i", this, function() { this.objects[0].acceleration[1] = 0; }, {'type':'keyup'} );
      	//	thrust left
      	controls.add( "j", this, function() { this.objects[0].acceleration[0] = -10; });
      	controls.add( "j", this, function() { this.objects[0].acceleration[0] = 0; }, {'type':'keyup'} );
      	//	thrust up
      	controls.add( "k", this, function() { this.objects[0].acceleration[1] = 10; });
      	controls.add( "k", this, function() { this.objects[0].acceleration[1] = 0; }, {'type':'keyup'} );
      	//	thrust right
      ...
      'update_strings'( debug_screen_object )   // Strings that this Scene_Component contributes to the UI:
      { 
      	debug_screen_object.string_map["FPS"] = "FPS: " + Math.round(1/this.mspf * 1000);
      },
    'display'( graphics_state )
      {
        // *** Lights: *** Values of vector or point lights over time.  Two different lights *per shape* supported; more requires changing a number in the vertex shader.
        ...
        for(var i = 0; i < this.objects.length; i++) {
        	var model_transform = this.objects[i].matrix;

        	// physics calculations
        	for(var j = 0; j < 3; j++) {
        		this.objects[i].velocity[j] += this.objects[i].acceleration[j] * delta_t/1000;
        	}
        	// rotations
        	model_transform = mult(model_transform, rotation(this.objects[i].ang_vel[0] * delta_t/1000, 1, 0, 0));
        	model_transform = mult(model_transform, rotation(this.objects[i].ang_vel[1] * delta_t/1000, 0, 1, 0));
        	model_transform = mult(model_transform, rotation(this.objects[i].ang_vel[2] * delta_t/1000, 0, 0, 1));
